...
static uint8_t ones_enc[] = 
    ...
    "\x67\x42\xfb\x16\x28\xa1\x93\xe6\x10\x94\x25\x4a\x57\x46\x7c\x0b";
...
void aes_ctr_encrypt(uint8_t * outputb, uint8_t * inputb, int size, uint8_t * key, uint8_t * iv, word_t * rk)
{
    ...
    for (i = 0; i < blocks; i++)
    {
        ...
        if (i > (BS_DATA_ROUNDS-1))
        {
            // 13 cycles/byte
            check_cfault(state);
            bs_get_slice(state, (word_t*)(outputb + offset), 28);
            offset += 16;
